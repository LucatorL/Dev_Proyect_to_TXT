          ...
          <ul className="space-y-2">
            {recents.map((recent) => (
              <li key={recent.id} className="flex items-center justify-between p-2 rounded-md hover:bg-secondary group">
                <Button
                  variant="link"
                  className="p-0 h-auto text-sm text-left flex-grow text-foreground hover:text-primary truncate"
                  onClick={() => onSelectRecent(recent)}
                  title={`Abrir: ${recent.name}\nTipo: ${recent.type}\nÚltima vez: ${new Date(recent.timestamp).toLocaleString()}`}
                >
                  {getIcon(recent.type)}
                  {recent.name}
                </Button>
                ...
                <AlertDialog>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        ¿Estás seguro de que quieres eliminar "{recent.name}" de la lista de recientes? Esta acción no se puede deshacer.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
